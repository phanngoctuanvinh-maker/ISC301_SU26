...
*   **API Specification:** Swagger (OpenAPI 3.0) được lưu ở tệp `openapi_spec.yaml` và cấu hình trực quan qua đường dẫn `/swagger`.
...
```text
ISC301_SU26/
├── docker/                    # Cấu hình container và database khởi tạo
│   └── mysql/
│       └── init.sql           # Schema SQL khởi tạo cơ sở dữ liệu và dữ liệu mẫu
├── uploads/                   # Thư mục lưu trữ tệp tin upload lên server
│   ├── avatars/               # Ảnh đại diện của người dùng
│   ├── brands/                # Ảnh logo của thương hiệu
│   └── categories/            # Ảnh biểu tượng của danh mục
├── server.js                  # Điểm khởi chạy ứng dụng (Entrypoint), nạp cấu hình và định tuyến
├── openapi_spec.yaml          # Tài liệu mô tả chuẩn API (OpenAPI 3.0)
└── src/                       # Thư mục chứa mã nguồn chính
    ├── config/                # Cấu hình hệ thống kết nối
    │   ├── db.js              # Cấu hình MySQL Pool & helper thực thi truy vấn
    │   ├── google.config.js   # Cấu hình Google Client ID cho OAuth2
    │   └── mail.js            # Cấu hình SMTP Nodemailer để gửi email
    ├── docs/                  # Tài liệu hóa API
    │   └── swagger.js         # Đăng ký và phục vụ giao diện tài liệu Swagger UI
    ├── jobs/                  # Các tác vụ chạy ngầm định kỳ
    │   └── otp.cleanup.job.js # Cron job dọn dẹp các OTP hết hạn khỏi DB
    ├── middlewares/           # Các bộ lọc trung gian xử lý request
    │   ├── auth.middleware.js # Middleware xác thực JWT token của người dùng
    │   ├── role.middleware.js # Middleware phân quyền (Admin / Customer)
    │   ├── upload.middleware.js # Cấu hình Multer lưu trữ và bọc xử lý lỗi upload
    │   └── validate.middleware.js # Middleware thực thi schema validation (Joi)
    ├── utils/                 # Các tiện ích dùng chung
    │   ├── email.templates.js # Mẫu giao diện Email gửi OTP bằng HTML
    │   ├── otp.util.js        # Sinh chuỗi OTP ngẫu nhiên 6 chữ số
    │   ├── response.util.js   # Định dạng chuẩn hóa API Response đầu ra
    │   └── slug.util.js       # Chuyển đổi chuỗi tiếng Việt thành slug thân thiện url
    └── modules/               # Tập hợp các module nghiệp vụ
        ├── auth/              # Module xác thực tài khoản (Register, Login, OTP, Google)
        ├── profile/           # Module thông tin cá nhân (Cập nhật, Đổi mật khẩu, Đổi avatar)
        ├── address/           # Module quản lý địa chỉ giao hàng (CRUD, Đặt mặc định)
        └── admin/             # Phân hệ quản trị viên (Admin)
            ├── brand/         # Quản lý thương hiệu (Brand CRUD)
            └── category/      # Quản lý danh mục dạng cây cha-con (Category tree & reorder)
```

Mỗi module nghiệp vụ (trong thư mục `src/modules/`) được cấu trúc khép kín bao gồm:
...
Dựa trên cấu trúc tệp [init.sql](file:///d:/giphub/ISC301_SU26/docker/mysql/init.sql), cơ sở dữ liệu bao gồm các bảng chính sau:
...
*   Import tệp schema khởi tạo tại: [docker/mysql/init.sql](file:///d:/giphub/ISC301_SU26/docker/mysql/init.sql).
