...
const addressService = {
  ...
  async createAddress(userId, body) {
    const { receiver_name, phone, address_line, ward, district, city, address_type } = body;
    ...
    const insertSql = `
      INSERT INTO addresses (user_id, receiver_name, phone, address_line, ward, district, city, address_type, is_default)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
    const params = [
      userId,
      ...
      district.trim(),
      city.trim(),
      address_type ? address_type.trim() : 'Nhà',
      isDefault
    ];
  ...
  async updateAddress(userId, addressId, body) {
    ...
    const allowedFields = ['receiver_name', 'phone', 'address_line', 'ward', 'district', 'city', 'address_type'];
    const fields = [];
    const values = [];
