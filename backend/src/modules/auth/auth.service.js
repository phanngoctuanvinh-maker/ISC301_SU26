...
const { otpEmailTemplate, resetPasswordEmailTemplate } = require('../../utils/email.templates');
...
const authService = {
  ...
  async forgotPassword(body) {
    ...
    const user = await db.queryOne('SELECT * FROM users WHERE email = ?', [email]);
    if (!user) {
      throw { status: 404, message: 'Email này không tồn tại trong hệ thống' };
    }

    // 2. Kiểm tra nếu user đăng ký bằng Google (không có mật khẩu)
    if (!user.password_hash) {
      throw { status: 400, message: 'Tài khoản này được đăng ký thông qua Google/Facebook, vui lòng đăng nhập bằng Google/Facebook' };
    }
    ...
    // 4. Sinh JWT Reset Token (hết hạn trong 10 phút)
    const token = jwt.sign(
      { userId: user.id, email: user.email, type: 'reset-password' },
      process.env.JWT_SECRET,
      { expiresIn: '10m' }
    );
    ...
    try {
      ...
      throw { status: 500, message: 'Không thể gửi email khôi phục mật khẩu, vui lòng thử lại' };
    }

    return { message: 'Đường dẫn đặt lại mật khẩu đã được gửi về email của bạn' };
  },
  ...
  async resetPassword(body) {
    ...
    try {
      ...
      throw { status: 400, message: 'Liên kết khôi phục mật khẩu không hợp lệ hoặc đã hết hạn' };
    }

    // 2. Kiểm tra type token
    ...
    if (!user) {
      throw { status: 404, message: 'Người dùng không tồn tại hoặc đã bị xóa' };
    }

    // 4. Mã hóa mật khẩu mới
    const hashedPassword = await bcrypt.hash(password, 10);

    // 5. Cập nhật mật khẩu mới trong DB
